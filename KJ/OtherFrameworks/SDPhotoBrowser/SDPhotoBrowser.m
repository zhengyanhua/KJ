...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.backgroundColor = [UIColor colorWithRed:0/255.f green:0/255.f blue:0/255.f alpha:0.8];
    }
    return self;
}
...
#pragma mark 修改bug
    ...
    [UIView animateWithDuration:SDPhotoBrowserHideImageAnimationDuration animations:^{
        ...
        if (self.deletePhotoBlock) {
            self.deletePhotoBlock(_currentImageIndex);
        }
    }];
}
...
- (void)layoutSubviews
{
    ...
    _saveButton.frame = CGRectMake(30, self.bounds.size.height - 70, 60, 30);
}
...
- (NSURL *)highQualityImageURLForIndex:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:highQualityImageURLForIndex:)]) {
        urlStr = [[self.delegate photoBrowser:self highQualityImageURLForIndex:index] absoluteString];
