...
- (BOOL)is3_5Inch_Ext
{
    return ([UIScreen instancesRespondToSelector:@selector(currentMode)] ? CGSizeEqualToSize(CGSizeMake(640, 960), [[UIScreen mainScreen] currentMode].size) : NO) || ([UIScreen instancesRespondToSelector:@selector(currentMode)] ? CGSizeEqualToSize(CGSizeMake(320, 640), [[UIScreen mainScreen] currentMode].size) : NO);
...
{//uuid+随机字符或数字 经过md5处理
   ...
   __autoreleasing NSString * endid=[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    ...
    if (endid.length==0) {
        ...
        string=(NSMutableString *)[string toMD5];
        endid=string;
