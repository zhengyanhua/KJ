...
static const char *FTKeyboardMargin = "FTKeyboardMargin";
...
- (void)setKeyboardMargin:(CGFloat)keyboardMargin
{
    objc_setAssociatedObject(self, FTKeyboardMargin, @(keyboardMargin), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CGFloat)keyboardMargin
{
    return [objc_getAssociatedObject(self, FTKeyboardMargin) floatValue];
}
...
- (void)TPKeyboardAvoiding_keyboardWillShow:(NSNotification*)notification {
    ...
    if (self.keyboardMargin) {
        self.contentInset = UIEdgeInsetsMake(self.contentInset.top, self.contentInset.left, self.contentInset.bottom + self.keyboardMargin, self.contentInset.right);
    }
